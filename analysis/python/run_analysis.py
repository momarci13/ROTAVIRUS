...
def macro(name: str, value, fmt: str = "{:,.0f}") -> None:
    """Register a LaTeX number macro (\\<name>)."""
    if isinstance(value, str):
        s = value
    else:
        s = fmt.format(value).replace(",", "\\,")
    NUM[name] = s
...
def write_table(path, df, header, aligns, fmts, caption, label):
...
write_table(TABDIR / "t2_cost_parameters.tex", cp_out,
            ...
            "Minden érték az elemzési réteg átlátható feltevése (lásd 3.4).",
            "tab:cost")
